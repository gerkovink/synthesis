...
### What do we want to plot.... exactly??
imp %>% complete("all") %>% map(~.x %$% lm(sws ~ log10(bw) + odi) %>% .$fitted.values) %>% bind_rows(.id="id") %>% summarize_at(vars(-id), var)

imp %>% complete("all") %>% map(~.x %$% lm(sws ~ log10(bw) + odi) %>% .$fitted.values) %>% 
  bind_cols() %>%
  rowwise() %>%
  mutate(means = mean(c_across()),
         vars = var(c_across()))
